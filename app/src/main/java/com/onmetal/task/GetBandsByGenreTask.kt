...
class GetBandsByGenreTask(private val searchBandsLoader: SearchBandsLoader) : AsyncTask<String, Unit, SearchResults?>() {

    override fun doInBackground(vararg params: String?): SearchResults? {
        val url = buildGetSearchBandsByGenreUrl(params[0], params[1])
    ...
    override fun onPostExecute(result: SearchResults?) {
        ...
        searchBandsLoader.loadSearchResultsActivity(result, true)
    }
}
